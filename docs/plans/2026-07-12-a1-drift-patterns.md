...
`~/.claude/rules/review/harness-engineering.md` の Drift Patterns 表（11 種のエージェント失敗モードと対策）を独立コンテンツとして切り出し、本リポジトリの README 旗艦に据える。Verifier Theater・Phantom Confabulation・Emergent menu drift は他 OSS に類例がなく、最大の差別化資産。

## 抽出元資産

- `~/.claude/rules/review/harness-engineering.md` — Drift Patterns 表（11 行）、Five Pillars、Autonomy Rollout Ladder
- 各パターンの出典メモリ（`reference_*`）— 公開時は外部ソース名に展開する

## 公開物

1. `README.md`（リポジトリ直下）— 11 パターンの一覧表 + 各パターンへのリンク。英語
2. `patterns/<slug>.md` × 11 — 症状 / 実例 / ハーネス対策 / 出典。英語
3. `plugins/drift-patterns/` — 対策をルールファイル化した Claude Code プラグイン（`rules/` として読み込める形）
...
4. プラグイン化: 対策ルールを `plugins/drift-patterns/rules/` に配置し `plugin.json` を書く
